 ...
 */
@Component
public class SystemRoleCodeRepeatFilter implements Filter<UserRole> {
    ...
    public void doFilter(UserRole context, FilterInvoker nextFilter) {
        if (null == context.getId()){
            ...
            if (StringUtils.isNotEmpty(context.getRoleCode())){
                if (!systemRole.getRoleCode().equals(context.getRoleCode())){
                    validateFilter(context.getRoleCode());
                }
            }
        }
    ...
    private void validateFilter(String roleCode){
        if (roleMapper.countUserRoleSizeByCode(roleCode) > 0 ){
            throw new BizException("ROLE_ADD_OR_EDIT_ERROR","当前权限编码已存在");
        }
    }
}
