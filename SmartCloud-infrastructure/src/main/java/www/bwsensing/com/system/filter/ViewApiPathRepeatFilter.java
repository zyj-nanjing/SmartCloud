 ...
 */
@Component
public class ViewApiPathRepeatFilter implements Filter<SystemMenu> {
    ...
    public void doFilter(SystemMenu context, FilterInvoker nextFilter) {
        if (StringUtils.isNotEmpty(context.getApiPath())){
            if (null == context.getId()){
                validateFilter(context);
            } else {
                ViewMenuDO currentView = viewMenuMapper.selectViewMenuById(context.getId());
                if (StringUtils.isNotEmpty(currentView.getApiPath())){
                    if (!currentView.getApiPath().equals(context.getApiPath())){
                        validateFilter(context);
                    }
                }
            }
    ...
    private void validateFilter(SystemMenu context){
        ...
        queryData.setApiPath(context.getApiPath());
        List<ViewMenuDO> menuList = viewMenuMapper.selectViewMenuList(queryData);
        if (menuList.size() > 0 ){
            throw new BizException("VIEW_MENU_ADD_OR_EDIT_ERROR","当前API请求路径已存在");
        }
    }
}
