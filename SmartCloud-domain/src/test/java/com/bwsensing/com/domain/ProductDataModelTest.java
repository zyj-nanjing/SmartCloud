...
public class ProductDataModelTest {
    ...
    private static final String ASCII_ACC_EXPECT = "MonitorReceive(sn=100000005, channelId=null, ip=null, receiveTime=null, " +
            ...
            "MonitorData(groupId=null, dataId=elect, dataIdValue=9.0, sn=100000005, type=null, timeStamp=null), " +
            "MonitorData(groupId=null, dataId=temp, dataIdValue=27.0, sn=100000005, type=null, timeStamp=null)])";
    ...
    private static final String HEX_ACC_EXPECT = "MonitorReceive(sn=000000001, channelId=null, ip=null, receiveTime=null, phoneNumber=null," +
            ...
            "MonitorData(groupId=null, dataId=temp, dataIdValue=22805.0, sn=000000001, type=null, timeStamp=null), " +
            "MonitorData(groupId=null, dataId=elect, dataIdValue=9.0, sn=000000001, type=null, timeStamp=null)])";
    ...
    public void testProductDataModelTransform(){
        ...
        Assert.assertEquals("BW ASCII 不含加速度解析错误", ASCII_NO_ACC_EXPECT, ASCII_WITH_NO_ACC.messageAnalyse(testNoAccAscii,new Timestamp(System.currentTimeMillis())).toString());
        String testAccAscii = "100000005,+003.4982,+000.5570,+086.4531,+000.0601,+000.0097,+000.9823,+027.0000,9,1440033931560";
        Assert.assertEquals("BW ASCII 含加速度解析错误", ASCII_ACC_EXPECT, ASCII_WITH_ACC.messageAnalyse(testAccAscii,new Timestamp(System.currentTimeMillis())).toString());
        String hexWithAcc ="01 03 10 4E DF 4E 60 70 7E 4F 6A 4E 8E 74 D3 59 15 00 09 9D E7";
        Assert.assertEquals("BW HEX 含加速度解析错误", HEX_ACC_EXPECT, HEX_MODEL.messageAnalyse(hexWithAcc,new Timestamp(System.currentTimeMillis())).toString());
    }
}
